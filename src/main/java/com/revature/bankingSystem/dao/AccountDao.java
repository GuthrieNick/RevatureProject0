...
public class AccountDao {
	...
	public static List<Account> getUserAccounts(int user_id) {
		...
		try {
			...
			while (rs.next())
				user_accts.add(
						new Account(rs.getInt(1), rs.getInt(1), Account.Type.values()[rs.getInt(3)], rs.getDouble(4)));
	...
	/**
	 * Transfer remaining funds from one account to another before removing it from the database.
	 * 
